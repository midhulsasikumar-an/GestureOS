...
| Checkpoint 3 — Gesture Recognition Layer | CP-2 (Gesture Recognition), recognition portion + stability/cooldown | Second half of PRD CP-2, isolated per the same rationale. |
...
| Checkpoint 5 — Action Layer | CP-3 (System Control) | Direct mapping, includes CursorController (TRD §3.14) per PRD §8.4/§8.5. |
| Checkpoint 6 — Context Engine | CP-4 (Context-Aware Engine) | Direct 1:1 mapping, includes Context Verification Layer (PRD §8.7.3). |
...
| Checkpoint 9 — Testing & Optimization | CP-7 (Optimization & Release), testing portion | Direct mapping for the testing/performance half of PRD CP-7. |
| Checkpoint 10 — Packaging & Deployment | CP-7 (Optimization & Release), packaging portion | Direct mapping for the packaging/deployment half of PRD CP-7, per PRD §20 Deployment Requirements. |
...
| System Command Engine (PRD §8.6) | Checkpoint 5 |
...
| Performance Budgets (PRD §16) | Checkpoint 9 |
| Deployment Requirements (PRD §20) | Checkpoint 10 |
...
**In scope:** all 14 PRD-defined gestures, `GestureEngine` orchestration, `StabilityFilter`, `CooldownFilter`.
...
1. `gestures/static_recognizer.py` — all 8 static gesture rules (Section 7.1 below), built on `gesture_utils.finger_angle()` and `finger_states()` from Checkpoint 2
2. `gestures/dynamic_recognizer.py` — all 6 dynamic gesture rules (Section 7.2 below), built on `MotionHistoryBuffer` from Checkpoint 2
3. `gestures/gesture_engine.py` — `GestureEngine` per TRD §3.9, orchestrating static + dynamic evaluation and returning the highest-confidence raw candidate per hand
4. `gestures/stability_filter.py` — `StabilityFilter` per TRD §3.10 (PRD §8.2 / FR-GS-01–04)
5. `gestures/cooldown_filter.py` — `CooldownFilter` per TRD §3.11 (PRD §8.3 / FR-CD-01–03)
6. Settings schema fields consumed: `gesture_confidence_threshold`, `gesture_stability_window_ms`, `gesture_cooldown_static_ms`, `gesture_cooldown_dynamic_ms`, `dynamic_window_ms` (all already present in the Checkpoint 0 schema; first consumed here)
7. Diagnostics logging extended with the `gesture` category: candidate detected, stability passed/failed, cooldown suppressed/passed (TRD §9.1 example log lines)
...
gestureos/
├── gestures/
│   ├── static_recognizer.py        # all 8 static gesture functions
│   ├── dynamic_recognizer.py       # all 6 dynamic gesture functions
│   ├── gesture_engine.py
│   ├── stability_filter.py
│   └── cooldown_filter.py
└── tests/
...
- All 14 gestures implemented exactly per their PRD §4.3/§4.4 rule summary and TRD §4 normalization discipline
...
- `StabilityFilter` and `CooldownFilter` pass their TRD §13.5 reference tests
...
3. `actions/executors/base.py` + three platform implementations — `CommandExecutor` ABC and `WindowsExecutor`/`MacOSExecutor`/`LinuxExecutor` per TRD §11.3, covering: mouse (click/double-click/drag/scroll), keyboard (hotkeys + single keys via context-managed press/release pairs to avoid stuck-key bugs per TRD §9.3 risk row), volume/brightness (platform-specific: pycaw/osascript/amixer), screenshot, lock screen, app launch
...
5. **System Engine last.** Volume/brightness/screenshot/lock-screen/app-launch are the most platform-divergent actions (each requiring entirely separate per-OS implementations: `pycaw` vs `osascript` vs `amixer`/`brightnessctl`) and are used by the fewest gestures in the default mapping set — sequencing them last means the dispatch architecture is already proven before tackling the highest platform-variance work.

### Modules

- `ActionEngine` (TRD §3.13)
- `CursorController` (TRD §3.14)
- `CommandExecutor` + platform implementations (TRD §11.3)
...
| Platform-specific volume/brightness APIs (pycaw, osascript, amixer) behave inconsistently or are unavailable in minimal OS installs | Each platform executor catches and logs its own platform-specific failures independently (TRD §3.13 Error Handling pattern); a missing `amixer` on a minimal Linux install degrades that one action, not the whole app | |
| OS permission denial (especially macOS Accessibility) blocks all dispatch | Per TRD §11.4 — caught per-action, logged ERROR, one-time user notification; this checkpoint must implement that catch-and-notify behavior, not just let dispatch silently fail | TRD §11.4 |
...
- All Acceptance Criteria pass on all three target platforms (or with platform-specific executors stubbed/mocked where CI lacks the target OS — full cross-platform validation may be partially deferred to Checkpoint 9's broader testing pass if CI infrastructure doesn't yet cover all 3 OSes, but this must be explicitly tracked, not silently skipped)
...
**In scope:** `ContextEngine` (TRD §3.12), the three platform `ContextAdapter` implementations, the `context_map.json` lookup table, the Context Verification Layer's 200ms hold-before-commit logic, and rewiring `ActionEngine.resolve()` to use real context instead of the Checkpoint 5 `'global'`-only stub.

**Out of scope:** Editing `context_map.json` via UI (Checkpoint 7's Mapping Editor, if in scope there — see Gap G-6 below).
...
2. `context/adapters/base.py` + three platform implementations — `ContextAdapter` ABC, `WindowsContextAdapter` (pywin32), `MacOSContextAdapter` (pyobjc/NSWorkspace), `LinuxContextAdapter` (Xlib, with documented Wayland degradation per TRD §9.2)
...
- `ContextAdapter` + 3 platform implementations (TRD §11.2)
...
| Wayland context-detection gap on Linux | Documented limitation (TRD §9.2/§16) — `LinuxContextAdapter` detects session type and logs a one-time INFO-level (not ERROR) notice that context-awareness degrades to `'global'`; this is accepted behavior, not a defect to fix in this checkpoint | TRD §16 Technical Risk: "Wayland context-detection gap" |
...
- Active window process name detected correctly on Windows, macOS, and Linux (Linux: X11 sessions; Wayland explicitly out of scope per documented limitation above)
...
- Wayland degradation is logged at INFO level exactly once per session, not repeatedly (verified by integration test asserting log-call count)
...
Per PRD §16 (Performance Budgets) and §21.1 (Product KPIs), all measured simultaneously on reference hardware (Intel Core i5, 8GB RAM, 720p webcam):
...
Produce the final, installable, native-executable build for all three target platforms, per PRD §20 Deployment Requirements and TRD §12 Packaging Strategy, and validate the release artifact end-to-end before it is considered shippable.

### Scope

**In scope:** PyInstaller spec finalization, per-platform installer creation (Inno Setup for Windows; `.dmg`/notarization for macOS; AppImage/`.deb` for Linux), the startup self-check (TRD §10.5/§12.4), and final release-artifact validation.

**Out of scope:** any application code changes — this checkpoint packages what Checkpoint 9 already validated; if packaging surfaces a defect (e.g., the MediaPipe bundling issue flagged as a risk since Checkpoint 1), the fix happens in the relevant component's code, but no new feature work occurs here.
...
3. macOS: code-signed, notarized `GestureOS.app` wrapped in a `.dmg` via `create-dmg` per TRD §12.3
4. Linux: AppImage (primary) and `.deb` (secondary) per TRD §12.3
5. CI build pipeline per TRD §12 implementing the three-platform matrix build (Windows/macOS/Linux runners, each producing their respective artifact)
6. Startup self-check (TRD §10.5/§12.4) verifying on first launch: MediaPipe model file present, ≥1 camera enumerable, `~/.gestureos/` writable, default mapping/profile JSON files copied successfully — with a specific diagnostic dialog (not a generic crash) on any failure
7. Final Release Deliverables package per PRD §20.3: the packaged executable/installer, user documentation, gesture reference card, calibration walkthrough, and default configuration files bundled and verified to be copied into `~/.gestureos/` on first launch
...
└── .github/workflows/ (or equivalent CI config)
    └── release_build.yml             # 3-platform matrix build per TRD §12
```
...
| macOS notarization failures (Apple tooling rejects the signed binary) | Build the code-signing/notarization step into the CI pipeline early in this checkpoint's work (not as a last-minute manual step) so failures surface with time to address them before a release deadline | |
| Linux packaging fragmentation (AppImage works on some distros, fails on others due to glibc version mismatches) | Test the AppImage artifact against at least Ubuntu 20.04 and one newer LTS release per PRD's stated minimum target (Ubuntu 20.04+) | PRD §1.2 target platforms |
...
Manual validation: full install → first-run onboarding → calibration → basic gesture-control session on a clean (non-developer) machine for each of the three target platforms, performed by someone other than the implementing engineer to catch assumptions baked in from familiarity with the dev environment.

### Definition of Done

- All three platform artifacts build successfully in CI
- Startup self-check passes on a clean machine (no prior GestureOS install, no prior Python install) for all three platforms
- Performance Budgets re-verified against the packaged artifact specifically
- All PRD §20.3 Release Deliverables are present and verified
- PRD §20.4 Release Acceptance Gate is fully satisfied — this is the final gate of the entire Implementation Plan
## 15. Risk Management
...
| CPU Usage | Medium | Medium (depends on target hardware tier) | Performance Budget — <20% target, with FPS/latency prioritized over CPU if conflict arises (PRD §16) | Full performance-budget test suite (Checkpoint 9) | CP 9 |
...
| User Fatigue (Gorilla Arm) | Medium | Medium (usage-pattern dependent, not directly software-mitigable) | Gestures designed for desk-height use; usage break reminders (PRD §18) | UAT subjective feedback only — **GAP G-7:** neither PRD nor TRD specifies a concrete "usage break reminder" mechanism (trigger condition, UI, frequency); flagged for product-owner decision rather than invented here | CP 9 (UAT only) |
| OS API Compatibility | Low | Low (mitigated architecturally from the start) | Platform-specific executor/adapter pattern (TRD §11) | Cross-platform integration testing (Checkpoint 5/6/9) | CP 5/6/9 |
| Performance on Low-End Hardware | Medium | Medium | Low-res mode, configurable FPS target (PRD §16, TRD §15) | Performance budget testing on reference hardware; low-end hardware testing not explicitly scoped — **GAP G-8:** PRD's reference hardware (Intel Core i5, 8GB RAM) is a mid-tier, not low-end, spec; no explicit low-end test machine is specified by either source document | CP 9 |
| Wayland Context-Detection Gap | Medium (Linux-specific) | High on pure-Wayland sessions, N/A on X11/Windows/macOS | Documented limitation, INFO-level one-time log, degrades to `'global'` context | Wayland-session manual test (Checkpoint 6) | CP 6 |
